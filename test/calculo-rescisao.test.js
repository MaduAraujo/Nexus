...
// calculo-rescisao.js roda via <script> no navegador e termina com `window.X = ...`;
// no Node, basta window existir para essas atribuições não lançarem erro.
global.window = global;

const { calcularRescisao, getDivisorHoraMensal } = require('../src/javascript/calculo-rescisao.js');

describe('justa_causa', () => {
    test('sem 13º, sem férias proporcionais, sem aviso, sem multa FGTS', () => {
        const r = calcularRescisao({
            tipo: 'justa_causa', salario: 3000,
            admissao: new Date(2020, 0, 10), demissao: new Date(2026, 2, 15),
        });
...
describe('sem_justa_causa', () => {
    test('aviso prévio integral + 13º/férias proporcionais projetados + multa de 40% do FGTS', () => {
        const r = calcularRescisao({
            tipo: 'sem_justa_causa', salario: 3000,
            admissao: new Date(2024, 0, 10), demissao: new Date(2026, 0, 20),
        });
        assert.equal(r.anosCompletos, 2);
        assert.equal(r.diasAviso, 36); // 30 + 3*2 anos completos
...
describe('pedido_demissao', () => {
    test('sem aviso, sem multa FGTS, e desconta saldo negativo de banco de horas', () => {
        const r = calcularRescisao({
            tipo: 'pedido_demissao', salario: 3000,
            admissao: new Date(2024, 0, 10), demissao: new Date(2026, 0, 20),
            saldoBancoHorasMin: -120, jornadaMin: 480,
        });
        assert.equal(r.diasAviso, 0);
...
describe('acordo_mutuo', () => {
    test('aviso prévio parcial (50%), multa de 20% do FGTS, e paga saldo positivo de banco de horas', () => {
        const r = calcularRescisao({
            tipo: 'acordo_mutuo', salario: 3000,
            admissao: new Date(2024, 0, 10), demissao: new Date(2026, 0, 20),
            saldoBancoHorasMin: 120, jornadaMin: 480,
        });
        assert.equal(r.diasAviso, 18); // metade do aviso integral de 36 dias
...
describe('getDivisorHoraMensal', () => {
    ...
    test('deriva o divisor da jornada real para jornada de 8h/dia (5 dias/semana)', () => {
        // 8h/dia * 5 dias = 40h semanais; 40 * 5 = 200
...
test('regra geral: custoTotal nunca é menor que a soma de verbas + encargos individuais', () => {
    const r = calcularRescisao({
        tipo: 'sem_justa_causa', salario: 5000,
        admissao: new Date(2021, 5, 1), demissao: new Date(2026, 6, 10),
    });
