...
const PALETTE = { indigo:'#6366f1', indigoDeep:'#4338ca', violet:'#8b5cf6', sky:'#38bdf8', amber:'#f59e0b', rose:'#f43f5e', teal:'#14b8a6', slate:'#64748b' };
...
function deptLabel(name) { return DEPT_LABEL_SHORT[name] || name; }
...
function setupChartTheme() {
    ...
    Chart.register({
        ...
        afterDraw(chart) {
            ...
            ctx.font = "700 24px 'Plus Jakarta Sans', sans-serif"; ctx.fillStyle = '#1f2937';
            ctx.fillText(cfg.value, cx, cy - 9);
            ctx.font = "600 10px 'Plus Jakarta Sans', sans-serif"; ctx.fillStyle = '#9ca3af';
...
async function loadData() {
    ...
    employees = (empData || []).map(e => ({
        ...
        birthDate: e.birth_date, gender: e.gender,
        salary: e.salary, pcd: e.pcd, racaCor: e.raca_cor,
    }));
...
function updateMetrics() {
    ...
    if (deltaAtivosEl)   { deltaAtivosEl.textContent   = admThisMonth > 0 ? `+${admThisMonth} admissão(ões) este mês` : '';     deltaAtivosEl.className   = admThisMonth > 0 ? 'card-delta up' : 'card-delta'; }
    if (deltaFeriasEl)   { deltaFeriasEl.textContent   = feriasThisMonth > 0 ? `${feriasThisMonth} início(s) este mês` : ''; deltaFeriasEl.className   = 'card-delta'; }
    if (deltaInativosEl) { deltaInativosEl.textContent = termThisMonth > 0 ? `+${termThisMonth} desligamento(s) este mês` : '';   deltaInativosEl.className = termThisMonth > 0 ? 'card-delta down' : 'card-delta'; }
}
...
async function updateAbsenteeism() {
    ...
    if (!totalWorkDays) { el.textContent = '—'; return; }
...
function updateDepartmentChart() {
    ...
    if (chartDepartment) { chartDepartment.destroy(); chartDepartment = null; }
    ...
    chartDepartment = new Chart(canvas, {
        ...
        data: {
            labels,
            datasets: [{
                data, backgroundColor: horizontalGradient('#a5b4fc', PALETTE.indigo),
                borderRadius: 6, borderSkipped: false, barPercentage: 0.62, categoryPercentage: 0.8
            }]
        },
        options: {
...
function updateTurnoverChart() {
    ...
    chartTurnover = new Chart(canvas, {
        ...
        data: {
            labels,
            datasets: [
                { label:'Admissões', data:admissions, backgroundColor: verticalGradient('#4ade80', '#16a34a'), borderRadius:6, borderSkipped:false, maxBarThickness:26 },
                { label:'Desligamentos', data:terminations, backgroundColor: verticalGradient('#fca5a5', '#dc2626'), borderRadius:6, borderSkipped:false, maxBarThickness:26 }
            ]
        },
        options: {
...
function updateTenureChart() {
    ...
    employees.filter(e => e.status === 'Ativo' && e.admissionDate).forEach(e => {
    ...
    if (chartTenure) { chartTenure.destroy(); chartTenure = null; }
    chartTenure = new Chart(canvas, {
        type: 'bar',
        data: { labels, datasets: [{ data, backgroundColor: barColors, borderRadius: 6, borderSkipped: false, barPercentage: 0.62, categoryPercentage: 0.85 }] },
        options: {
...
function updateAgeChart() {
    ...
    employees.filter(e => e.status === 'Ativo' && e.birthDate).forEach(e => {
...
function updateGenderChart() {
    ...
    const counts = activeEmployees.reduce((acc, e) => { acc[e.gender] = (acc[e.gender] || 0) + 1; return acc; }, {});
...
const RACA_COLORS = { 'Branca': PALETTE.sky, 'Preta': PALETTE.indigoDeep, 'Parda': PALETTE.amber, 'Amarela': PALETTE.teal, 'Indígena': PALETTE.rose, 'Prefiro não declarar': PALETTE.slate };
...
function updateGenderEquity() {
    ...
    const colorMap = { 'Masculino': PALETTE.sky, 'Feminino': PALETTE.rose, 'Outro': PALETTE.violet };
    const breakdown = genders.sort((a, b) => avg[b] - avg[a]).map(g =>
...
// Mesmo cálculo de updateGenderEquity(), mas cruzando raca_cor × salário — sem
// isso, o gráfico chart-race mostrava só representatividade (quantos), nunca
// se há disparidade salarial entre grupos (o que mais importa para compliance).
function updateRaceEquity() {
    ...
    const breakdown = races.sort((a, b) => avg[b] - avg[a]).map(r =>
...
function updateRaceChart() {
    ...
    const counts = activeEmployees.reduce((acc, e) => {
        const r = e.racaCor || 'Não Informado'; acc[r] = (acc[r] || 0) + 1; return acc;
    }, {});
    const entries = Object.entries(counts).sort((a, b) => a[1] - b[1]);
    const labels = entries.map(e => e[0]);
    const data = entries.map(e => e[1]);
...
function updatePayrollChart() {
    ...
    if (!canvas || typeof Chart === 'undefined') return;
    const { labels, values } = last6Months((y, m, key) =>
        payslips.filter(p => p.mes === key).reduce((s, p) => s + (Number(p.salario_liquido) || 0), 0)
    );
    const withData = values.filter(v => v > 0);
    const avg = withData.length ? withData.reduce((s, v) => s + v, 0) / withData.length : 0;
    const fmt = v => v.toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    if (chartPayroll) { chartPayroll.destroy(); chartPayroll = null; }
    chartPayroll = new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Custo de folha', data: values,
                    borderColor: PALETTE.indigo, borderWidth: 2.5,
                    backgroundColor: ctx => {
                        const { chartArea } = ctx.chart;
                    ...
                    fill: true, tension: .4, pointRadius: 3.5, pointHoverRadius: 6,
                    pointBackgroundColor: '#fff', pointBorderColor: PALETTE.indigo, pointBorderWidth: 2, order: 2
                },
                {
                    label: 'Média (6 meses)', data: Array(labels.length).fill(avg),
                    borderColor: 'rgba(100,116,139,.55)', borderWidth: 1.5, borderDash: [6, 4],
        ...
        options: {
            ...
            scales: { y: { beginAtZero: true, ticks: { callback: v => `R$ ${(v / 1000).toFixed(0)}k` }, grid: { color: 'rgba(0,0,0,.05)' } }, x: { grid: { display: false } } }
        }
    });
}
...
function updateOvertimeChart() {
    ...
    const { labels, values } = last6Months((y, m) => {
        const minutes = bankAdjustments.filter(a => {
    ...
    if (chartOvertime) { chartOvertime.destroy(); chartOvertime = null; }
    chartOvertime = new Chart(canvas, {
        type: 'bar',
        data: { labels, datasets: [{ label: 'Horas creditadas', data: values, backgroundColor: verticalGradient('#fcd34d', PALETTE.amber), borderRadius: 6, borderSkipped: false, maxBarThickness: 30 }] },
        options: {
            responsive: true, maintainAspectRatio: true,
...
function updateDeptTurnoverChart() {
    ...
    if (chartDeptTurnover) { chartDeptTurnover.destroy(); chartDeptTurnover = null; }
    if (!labels.length) { canvas.style.display = 'none'; return; }
    canvas.style.display = '';
...
function readersByMessage() {
    const map = {};
    messageReads.forEach(r => { (map[r.message_id] || (map[r.message_id] = new Set())).add(r.employee_id); });
    return map;
}
...
function updateDeptReadRateChart() {
    ...
    const entries = Object.entries(activeByDept).map(([dept, empIds]) => {
    ...
    if (chartDeptReadRate) { chartDeptReadRate.destroy(); chartDeptReadRate = null; }
    if (!labels.length) { canvas.style.display = 'none'; return; }
    canvas.style.display = '';
...
function updateReadTimeChart() {
    ...
    const { labels, values } = last6Months((y, m) => {
        const hours = messageReads
            .filter(r => { const d = new Date(r.read_at); return d.getFullYear() === y && d.getMonth() === m; })
            .map(r => {
    ...
    if (chartReadTime) { chartReadTime.destroy(); chartReadTime = null; }
    chartReadTime = new Chart(canvas, {
        type: 'line',
        data: {
            labels,
            datasets: [{
                label: 'Tempo médio até leitura', data: values,
                borderColor: PALETTE.violet, borderWidth: 2.5,
                backgroundColor: ctx => {
                    const { chartArea } = ctx.chart;
                ...
                pointBackgroundColor: '#fff', pointBorderColor: PALETTE.violet, pointBorderWidth: 2
            }]
        },
        options: {
            responsive: true, maintainAspectRatio: true,
...
function updateLowAdoptionList() {
    ...
    container.innerHTML = lowest.map(m => {
...
function setupRealtimeSync() {
    sb.channel('dashboard-rt')
        ...
        .on('postgres_changes', { event:'*', schema:'public', table:'payslips' }, async () => { await loadData(); refreshAll(); })
        .on('postgres_changes', { event:'*', schema:'public', table:'bank_adjustments' }, async () => { await loadData(); refreshAll(); })
        .on('postgres_changes', { event:'*', schema:'public', table:'messages' }, async () => { await loadData(); refreshAll(); })
        .on('postgres_changes', { event:'*', schema:'public', table:'message_reads' }, async () => { await loadData(); refreshAll(); })
        .subscribe();
}
...
function exportToPDF() {
    ...
    const pageW = 210, pageH = 297, margin = 14, colGap = 6;
    ...
    function drawHeader() {
        doc.setFillColor(13, 14, 18); doc.rect(0, 0, pageW, headerH, 'F');
    ...
    const kpiW = colW, kpiH = 20, kpiGap = colGap;
    const ky = contentTop;
    ...
    function sectionTitle(title) {
        ...
        if (sy > contentBottom - 10) { doc.addPage(); drawHeader(); colY = [contentTop, contentTop]; col = 0; sy = contentTop; }
        doc.setFillColor(99, 102, 241); doc.roundedRect(margin, sy, 2, 6, 1, 1, 'F');
