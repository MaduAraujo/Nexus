...
// Compliance
let holidaysMap    = {};  // { 'YYYY-MM-DD': { id, name, abrangencia } }
let vacationsByEmp = {};  // { employee_id: [{ start_date, end_date, status }] }
let hrSettings     = { banco_horas_vencimento_meses: 6, limite_extra_diario_min: 120 };
let ledgerMap      = {};  // { employee_id: { status:'ok'|'atencao'|'vencido', minutosVencendo, minutosVencidos, proxExpira } }

// Solicitações (bank_requests)
let bankRequestsAll   = [];
...
function isSunday(dateKey) { return new Date(`${dateKey}T12:00:00`).getDay() === 0; }

function isNoturno(rec) {
    const hrs = [rec.entrada, rec.saida_almoco, rec.retorno_almoco, rec.saida]
        .filter(Boolean).map(v => new Date(v).getHours());
    return hrs.some(h => h >= 22 || h < 5);
}

// ─── Ledger de vencimento do banco de horas (FIFO por competência) ──
...
async function loadBankLedger() {
    ...
    const byEmpMonth = {}; // { employee_id: { 'YYYY-MM': netMin } }
    const empMap = {}; allEmps.forEach(e => { empMap[e.id] = e; });

    (allTime || []).forEach(r => {
        const emp = empMap[r.employee_id]; if (!emp) return;
        const jornadaMin = getJornadaMin(emp);
        if (jornadaMin === null) return; // PJ não participa do banco de horas
    ...
    (allAdj || []).forEach(a => {
        const emp = empMap[a.employee_id]; if (!emp) return;
    ...
    Object.entries(byEmpMonth).forEach(([empId, months]) => {
        ...
        const queue = []; // buckets de crédito ainda não compensados, em ordem cronológica (FIFO)
        keys.forEach(mk => {
            let net = months[mk];
            if (net > 0) { queue.push({ mk, remaining: net }); }
                ...
                while (debt > 0 && queue.length) {
                    ...
                    oldest.remaining -= consumed; debt -= consumed;
                    if (oldest.remaining <= 0) queue.shift();
                }
            }
        });

        if (!queue.length) { ledgerMap[empId] = { status: 'ok', minutosVencendo: 0, minutosVencidos: 0, proxExpira: null }; return; }

        let minutosVencendo = 0, minutosVencidos = 0, pior = 'ok', proxExpira = null;
        queue.forEach(bucket => {
            const [y, m] = bucket.mk.split('-').map(Number);
            const expira = new Date(y, (m - 1) + vencMeses, 1);
            const diasRestantes = Math.round((expira - hoje) / 86400000);
            if (diasRestantes < 0)      { minutosVencidos  += bucket.remaining; pior = 'vencido'; }
            else if (diasRestantes <= 30) { minutosVencendo += bucket.remaining; if (pior !== 'vencido') pior = 'atencao'; }
            if (proxExpira === null || expira < proxExpira) proxExpira = expira;
        });
        ledgerMap[empId] = { status: pior, minutosVencendo, minutosVencidos, proxExpira };
    });
}
...
async function loadAllData() {
    ...
    allEmps = (empData || []).map(e => ({
        ...
        admissionDate: e.admission_date, workLoad: e.work_load,
...
// exige 15min; até 4h (ou PJ) não há intervalo obrigatório.
function getIntervaloMinObrigatorio(jornadaMin) {
...
// Minutos de intervalo suprimidos num dia encerrado. Desde a Lei 13.467/2017 (art. 71 §4º) isso
// é indenizado (tempo suprimido + 50%) à parte — não entra no banco de horas/saldo.
function calcIntervaloDeficitMin(rec, jornadaMin) {
    if (isFalta(rec) || !rec.saida) return 0;
    const obrigatorio = getIntervaloMinObrigatorio(jornadaMin);
...
function computeBalance(emp, monthKey) {
    ...
    const limiteExtra = hrSettings.limite_extra_diario_min || 120;
    let extrasMin=0, faltaMin=0, diasCompletos=0, totalWorkedMin=0, diasExcesso=0, intervaloDeficitMin=0, diasIntervaloIrregular=0;
    Object.entries(records).forEach(([key, rec]) => {
        ...
        if (!isPJ) {
            ...
            if(s!==null){
                if(s>0){ extrasMin+=s; if (s>limiteExtra) diasExcesso++; }
                else faltaMin+=Math.abs(s);
            }
            const intervaloDeficit = calcIntervaloDeficitMin(rec, jornadaMin);
            if (intervaloDeficit>0) { intervaloDeficitMin+=intervaloDeficit; diasIntervaloIrregular++; }
        }
    });
    let ajusteMin=0;
    ...
    return { emp, jornadaMin, isPJ, extrasMin, faltaMin, ajusteMin, saldoLiquido, diasCompletos, totalWorkedMin, diasExcesso, intervaloDeficitMin, diasIntervaloIrregular, ledger };
}

// ─── KPIs ─────────────────────────────────────────────────────

function loadKPIs() {
    let totalExtras=0, totalFaltas=0, cntPos=0, cntNeg=0, cntCritico=0, cntVencendo=0, cntVencido=0, cntExcesso=0, cntIntervalo=0;
    allData.forEach(d => {
        if (d.isPJ) return;
...
function renderTable() {
    ...
    if (!filtered.length) { tbody.innerHTML=`<tr><td colspan="9"><div class="table-empty"><i class="fas fa-clock-rotate-left"></i><p>Nenhum colaborador encontrado.</p></div></td></tr>`; setText('table-count',''); return; }
...
function setupExportButton() {
    const btn = $('btn-export'), menu = $('export-menu'), chevron = $('export-chevron');
    if (!btn || !menu) return;
    const close = () => { menu.classList.remove('open'); chevron?.classList.remove('open'); };
    ...
    $('export-pdf-btn')?.addEventListener('click', () => { close(); exportToPDF(); });
    $('export-csv-btn')?.addEventListener('click', () => { close(); exportToCSV(); });
}
...
function exportToCSV() {
    ...
    const rows = filtered.map(d => [
        d.emp.name, d.emp.dept||'', d.isPJ?'PJ':(d.emp.contractType||'CLT').toUpperCase(),
        jornadaLabel(d.emp, d.jornadaMin), d.diasCompletos,
    ...
    const a = document.createElement('a'); a.href = url; a.download = `banco-horas-${currentMonth}.csv`;
...
function exportToPDF() {
    ...
    const cols = [
        { label:'Colaborador',    w:60 }, { label:'Tipo',        w:20 },
        { label:'Jornada',        w:28 }, { label:'Dias Reg.',   w:20 },
        { label:'H. Extras',      w:26 }, { label:'H. em Falta', w:26 },
        { label:'Saldo do Mês',   w:28 }, { label:'Compliance',  w:60 },
    ];

    function drawHeader() {
        doc.setFillColor(13, 14, 18); doc.rect(0, 0, pageW, 20, 'F');
    ...
    filtered.forEach(d => {
        if (y > pageH - 14) { doc.addPage(); drawHeader(); y = drawTableHeader(28); }
        let x = margin;
        const cells = [
            d.emp.name,
        ...
        cells.forEach((val, i) => { doc.text(String(val), x, y); x += cols[i].w; });
        y += 6;
    });

    if (!filtered.length) { doc.setFont('helvetica', 'italic'); doc.setTextColor(148,163,184); doc.text('Nenhum colaborador encontrado para os filtros atuais.', margin, y); }
...
function buildComplianceBadges(d) {
    ...
    const badges = [];
    if (d.ledger?.status==='vencido')       badges.push(`<span class="badge badge--vencido" title="Banco de horas vencido — vira passivo trabalhista"><i class="fas fa-hourglass-end"></i> Vencido</span>`);
    else if (d.ledger?.status==='atencao')  badges.push(`<span class="badge badge--vencendo" title="Banco de horas vencendo em até 30 dias"><i class="fas fa-hourglass-half"></i> Vencendo</span>`);
    if (d.diasExcesso>0)                    badges.push(`<span class="badge badge--excesso" title="Dias com mais de 2h extras (limite do art. 59 da CLT)"><i class="fas fa-stopwatch"></i> ${d.diasExcesso}x 2h+</span>`);
    if (d.diasIntervaloIrregular>0)         badges.push(`<span class="badge badge--intervalo" title="Intervalo intrajornada não cumprido (art. 71 CLT) — ${minToStr(d.intervaloDeficitMin)} suprimidos, indenizáveis com adicional de 50%"><i class="fas fa-mug-saucer"></i> ${d.diasIntervaloIrregular}x intervalo</span>`);
...
function buildRow(d) {
    ...
    const jStr=jornadaLabel(emp,jornadaMin),ctStr=emp.contractType||'CLT';
...
const FILTER_LABELS_BH = { todos: 'Filtro', positivo: 'Saldo Positivo', negativo: 'Saldo Negativo', zerado: 'Zerado / PJ', critico: 'Crítico', vencendo: 'Banco Vencendo', vencido: 'Banco Vencido' };
...
function renderDetailModal(emp, monthKey) {
    ...
    const jornadaMin=getJornadaMin(emp), isPJ=jornadaMin===null;
    ...
    let extrasMin=0,faltaMin=0,diasCompletos=0,totalWorked=0,intervaloDeficitMin=0,diasIntervaloIrregular=0;
    monthRecs.forEach(([,rec])=>{if(isFalta(rec)||!rec.saida)return;diasCompletos++;totalWorked+=calcWorkedMin(rec);if(!isPJ){const s=calcSaldoMin(rec,jornadaMin);if(s!==null){if(s>0)extrasMin+=s;else faltaMin+=Math.abs(s);}const dInt=calcIntervaloDeficitMin(rec,jornadaMin);if(dInt>0){intervaloDeficitMin+=dInt;diasIntervaloIrregular++;}}});
    ...
    if(!isPJ){
        ...
        if(diasIntervaloIrregular>0){
            const indenizacaoMin=Math.round(intervaloDeficitMin*1.5);
    ...
    else { html+=`<div class="detail-table-wrap"><table class="detail-table"><thead><tr><th>Data</th><th>Entrada</th><th>Saída Alm.</th><th>Retorno</th><th>Saída</th><th>Trabalhado</th><th>Saldo</th><th>Status</th></tr></thead><tbody>${monthRecs.map(([key,rec])=>buildDayRow(key,rec,jornadaMin,isPJ,emp.id)).join('')}</tbody></table></div>`; }
    ...
    else { html+=monthAjustes.map(a=>`<div class="ajuste-item"><span class="ajuste-tipo-badge ${a.tipo}"><i class="fas ${a.tipo==='credito'?'fa-plus':'fa-minus'}"></i>${a.tipo==='credito'?'Crédito':'Débito'}</span><div class="ajuste-info"><p class="ajuste-valor">${a.tipo==='credito'?'+':'-'}${minToStr(a.minutos)}</p><p class="ajuste-just">${esc(a.justificativa)}</p><p class="ajuste-meta">${a.date} &bull; por ${esc(a.created_by_name)||'RH'} &bull; ${new Date(a.created_at).toLocaleDateString('pt-BR')}</p></div><button class="btn-delete-ajuste" onclick="deleteAjuste('${emp.id}','${a.id}')" title="Excluir ajuste"><i class="fas fa-trash"></i></button></div>`).join(''); }
    html+=`</div>`;
    body.innerHTML=html;
...
async function renderSaldoTrendChart(emp) {
    ...
    const monthly = {};
    for (let i=0;i<monthsBack;i++){ const d=new Date(startMonth.getFullYear(),startMonth.getMonth()+i,1); monthly[`${d.getFullYear()}-${pad0(d.getMonth()+1)}`]=0; }
    ...
    if (jornadaMin !== null) {
        ...
        (recs||[]).forEach(r=>{ if(isFalta(r)||!r.saida) return; const s=calcSaldoMin(r,jornadaMin); if(s===null)return; const mk=r.date.slice(0,7); if(mk in monthly) monthly[mk]+=s; });
        (adjs||[]).forEach(a=>{ const mk=a.date.slice(0,7); if(!(mk in monthly)) return; monthly[mk] += a.tipo==='credito'?a.minutos:-a.minutos; });
    }

    const keys   = Object.keys(monthly).sort();
    const labels = keys.map(k=>{ const [y,m]=k.split('-').map(Number); return new Date(y,m-1,1).toLocaleDateString('pt-BR',{month:'short',year:'2-digit'}); });
    const values = keys.map(k => +(monthly[k]/60).toFixed(1));

    if (detailTrendChart) { detailTrendChart.destroy(); detailTrendChart = null; }
    detailTrendChart = new Chart(canvas, {
        type: 'line',
        data: {
            labels,
            datasets: [{
                label: 'Saldo do mês (h)', data: values,
                borderColor: '#6366f1', borderWidth: 2.5,
                backgroundColor: ctx => {
                    const { chartArea } = ctx.chart; if (!chartArea) return 'rgba(99,102,241,.12)';
...
function buildDayRow(key, rec, jornadaMin, isPJ, empId) {
    ...
    const t=field=>{
        const v=rec[field];if(!v)return`<span class="dt-time missing">—</span>`;
        const dt=new Date(v);
        const selfiePath=rec[field+'_selfie_path'];
        const selfieBtn=selfiePath?`<button type="button" class="btn-selfie-view" onclick="viewPontoSelfie('${selfiePath}')" title="Ver selfie do registro"><i class="fas fa-camera"></i></button>`:'';
    ...
    const extraTags=[];
    if(holiday)            extraTags.push(`<span class="badge-sm badge-sm-feriado" title="${holiday.name} — adicional de 100% calculado no holerite do mês (Súmula 146 TST)">Feriado</span>`);
    else if(isSunday(key)) extraTags.push(`<span class="badge-sm badge-sm-domingo" title="Trabalho em domingo — adicional de 100% calculado no holerite do mês (Súmula 146 TST)">Domingo</span>`);
    if(isNoturno(rec))     extraTags.push(`<span class="badge-sm badge-sm-noturno" title="Horário noturno (22h-5h) — adicional de 20% calculado no holerite do mês (CLT art. 73)">Noturno</span>`);
    if(!isPJ){
        const intervaloDeficit = calcIntervaloDeficitMin(rec, jornadaMin);
        if(intervaloDeficit>0) extraTags.push(`<span class="badge-sm badge-sm-intervalo" title="Intervalo do art. 71 da CLT não cumprido — ${minToStr(intervaloDeficit)} devidos com adicional de 50%, pagos à parte">Intervalo -${minToStr(intervaloDeficit)}</span>`);
    }
    if(!rec.saida){badgeHTML=`<span class="badge-sm badge-sm-incompleto">Incompleto</span>`;}
        ...
        if(saldo!==null){saldoStr=(saldo>=0?'+':'-')+minToStr(saldo);saldoCls=saldo>0?'positivo':saldo<0?'negativo':'zero';}
        const limiteExtra=hrSettings.limite_extra_diario_min||120;
        if(rec.ajustado)              badgeHTML=`<span class="badge-sm badge-sm-incompleto">Ajustado</span>`;
        else if(saldo>limiteExtra)    badgeHTML=`<span class="badge-sm badge-sm-excesso" title="Excede o limite legal de 2h de horas extras diárias (art. 59 CLT)">Excesso 2h+</span>`;
...
window.submitAdjust = async function () {
    ...
    // Compliance preventivo: bloqueia lançamento de crédito que ultrapasse o limite diário
    // de horas extras configurado (CLT art. 59, §1º) em vez de só sinalizar com badge depois.
    if (tipo === 'credito') {
        const limiteExtra = hrSettings.limite_extra_diario_min || 120;
        const { data: dayCredits } = await sb.from('bank_adjustments')
            .select('minutos')
        ...
        if (existingMin + total > limiteExtra) {
            return showErr(`Limite legal de horas extras diárias excedido (CLT art. 59, §1º). Já lançado nesse dia: ${minToStr(existingMin)} + ${minToStr(total)} solicitado(s) > limite de ${minToStr(limiteExtra)}/dia.`);
        }
    }
    ...
    // Ajustes manuais do RH não são aplicados direto: viram uma solicitação pendente de
    // aprovação em duas etapas (gestor do colaborador, ou um segundo Administrador se não
    // houver gestor definido) — só vira um lançamento real em bank_adjustments quando aprovado.
    const { error } = await sb.from('bank_requests').insert({
        employee_id: adjustingEmpId, origem: 'rh', tipo, minutos: total, date: data,
        justificativa: just, requires_approval_from: requiresApprovalFrom,
        manager_id_snapshot: empData.emp.managerId || null,
        created_by_name: rhUser?.email?.split('@')[0] || 'RH', created_by_email: rhUser?.email,
    });

    if (error) { showErr('Erro ao enviar ajuste para aprovação.'); return; }

    closeModal('adjust-modal');
    const aprovadorMsg = requiresApprovalFrom === 'gestor' ? 'aguardando aprovação do gestor responsável' : 'aguardando confirmação de um segundo Administrador';
...
// ─── Notificações proativas (saldo crítico, banco vencido, solicitações) ──

function setupNotifPanel() {
    const btn = $('btn-notif'), panel = $('notif-panel'), wrapper = $('notif-wrapper');
    btn?.addEventListener('click', e => { e.stopPropagation(); panel?.classList.toggle('hidden'); });
    document.addEventListener('click', e => { if (wrapper && !wrapper.contains(e.target)) panel?.classList.add('hidden'); });
}

function renderNotifPanel() {
    const badge = $('notif-badge'), body = $('notif-panel-body');
    ...
    const rows = [];
    if (criticos.length) rows.push(`<div class="notif-item" onclick="goToCriticos()"><div class="notif-item-icon notif-item-icon--vencido"><i class="fas fa-triangle-exclamation"></i></div><div class="notif-item-body"><span class="notif-item-title">${criticos.length} colaborador${criticos.length>1?'es':''} em saldo crítico</span><span class="notif-item-sub">Mais de 20h negativas no banco de horas</span></div></div>`);
    if (vencidos.length) rows.push(`<div class="notif-item" onclick="goToVencidos()"><div class="notif-item-icon notif-item-icon--vencido"><i class="fas fa-hourglass-end"></i></div><div class="notif-item-body"><span class="notif-item-title">${vencidos.length} colaborador${vencidos.length>1?'es':''} com banco de horas vencido</span><span class="notif-item-sub">Prazo de compensação expirado</span></div></div>`);
    if (pendentes.length) rows.push(`<div class="notif-item" onclick="goToSolicitacoes()"><div class="notif-item-icon notif-item-icon--checklist"><i class="fas fa-inbox"></i></div><div class="notif-item-body"><span class="notif-item-title">${pendentes.length} solicitação${pendentes.length>1?'ões':''} de banco de horas pendente${pendentes.length>1?'s':''}</span><span class="notif-item-sub">Aguardando decisão</span></div></div>`);
...
function renderRequestsTab() {
    const tbody = $('requests-tbody'); if (!tbody) return;
    const filtered = reqFilter === 'todos' ? bankRequestsAll : bankRequestsAll.filter(r => r.status === reqFilter);
    if (!filtered.length) { tbody.innerHTML = `<tr><td colspan="8"><div class="table-empty"><i class="fas fa-inbox"></i><p>Nenhuma solicitação encontrada.</p></div></td></tr>`; setText('requests-count',''); return; }
    tbody.innerHTML = filtered.map(buildRequestRow).join('');
    const cnt = filtered.length; setText('requests-count', `${cnt} solicitação${cnt!==1?'ões':''} exibida${cnt!==1?'s':''}`);
}

function buildRequestRow(r) {
    const empName = r.employees?.name || '—', empDept = r.employees?.dept || '—';
    const origemBadge = r.origem === 'rh' ? `<span class="audit-badge audit-badge--rh">RH</span>` : `<span class="audit-badge audit-badge--colab">Colaborador</span>`;
    ...
    const anexoBtn = r.anexo_path ? `<button class="btn-icon btn-icon--view" onclick="viewRequestAnexo('${r.id}')" title="Ver anexo"><i class="fas fa-paperclip"></i></button>` : '';
...
window.viewPontoSelfie = async function (path) {
    ...
    if (error || !data?.signedUrl) { showToast('Não foi possível abrir a selfie.', 'error'); return; }
    window.open(data.signedUrl, '_blank');
    // Path é sempre `${employeeId}/${date}_${step}_${timestamp}.jpg` (ver ponto-colaborador.js).
    const empId = path.split('/')[0];
    if (empId) NexusAuth.logAccess(empId, 'selfie_ponto', path);
};

window.viewRequestAnexo = async function (id) {
    const r = bankRequestsAll.find(x => x.id === id); if (!r?.anexo_path) return;
...
window.approveRequest = async function (id) {
    const { error } = await sb.rpc('approve_bank_request', {
        ...
        p_decided_by_name: rhUser?.email?.split('@')[0] || 'RH', p_decided_by_email: rhUser?.email,
    });
    if (error) { showToast(error.message || 'Erro ao aprovar solicitação.', 'error'); return; }
    showToast('Solicitação aprovada.', 'success');
    await refresh();
    await loadBankRequests(); renderRequestsTab(); updatePendingBadge();
};

window.openRejectRequestModal = function (id) {
    rejectingRequestId = id;
    const r = bankRequestsAll.find(x => x.id === id);
    const sub = $('reject-request-sub'); if (sub) sub.textContent = r?.employees?.name || '';
    const obs = $('reject-request-obs'); if (obs) obs.value = '';
    const al  = $('reject-request-alert'); if (al) { al.className='modal-alert'; al.textContent=''; }
...
window.confirmRejectRequest = async function () {
    ...
    if (!obs) { if(al){al.className='modal-alert error';al.innerHTML=`<i class="fas fa-exclamation-circle"></i> Informe o motivo da rejeição.`;} return; }
    const { error } = await sb.rpc('approve_bank_request', {
        p_request_id: rejectingRequestId, p_decision: 'rejeitado', p_obs: obs,
        p_decided_by_name: rhUser?.email?.split('@')[0] || 'RH', p_decided_by_email: rhUser?.email,
    });
    if (error) { if(al){al.className='modal-alert error';al.textContent=error.message||'Erro ao rejeitar solicitação.';} return; }
...
window.openHolidaysModal = function () {
    const dt=$('holiday-date'); if(dt) dt.value='';
    const nm=$('holiday-name'); if(nm) nm.value='';
    renderHolidaysList();
    openModal('holidays-modal');
};

function renderHolidaysList() {
    const wrap=$('holidays-list'); if(!wrap) return;
    const list=Object.entries(holidaysMap).sort(([a],[b])=>a.localeCompare(b));
    if (!list.length) { wrap.innerHTML=`<p class="no-ajustes">Nenhum feriado cadastrado.</p>`; return; }
    wrap.innerHTML = list.map(([date,h]) => `<div class="ajuste-item"><span class="ajuste-tipo-badge credito">${HOLIDAY_ABR_LABEL[h.abrangencia]||h.abrangencia}</span><div class="ajuste-info"><p class="ajuste-valor" style="font-size:13px">${fmtDate(date)}</p><p class="ajuste-just">${h.name}</p></div><button class="btn-delete-ajuste" onclick="deleteHoliday('${h.id}')" title="Excluir"><i class="fas fa-trash"></i></button></div>`).join('');
}
...
window.submitHoliday = async function () {
    ...
    if (!date || !name) { showToast('Informe a data e o nome do feriado.', 'error'); return; }

    const { data, error } = await sb.from('holidays').insert({ date, name, abrangencia }).select().single();
    if (error) { showToast(error.code === '23505' ? 'Já existe um feriado cadastrado nesta data.' : 'Erro ao salvar feriado.', 'error'); return; }
...
window.openSettingsModal = function () {
    const v=$('settings-vencimento');   if(v) v.value = hrSettings.banco_horas_vencimento_meses || 6;
    const l=$('settings-limite-extra'); if(l) l.value = hrSettings.limite_extra_diario_min || 120;
    const al=$('settings-alert'); if(al){ al.className='modal-alert'; al.textContent=''; }
...
window.submitSettings = async function () {
    ...
    const { error } = await sb.from('hr_settings').update({
        banco_horas_vencimento_meses: vencimento, limite_extra_diario_min: limite, updated_at: new Date().toISOString(),
    }).eq('id', 1);
...
window.toggleStatsRow = function () {
    const row = $('stats-row'), chevron = $('stats-row-chevron'), btn = $('stats-row-toggle');
...
window.switchTab = function (btn, name) { document.querySelectorAll('.tabs-bar .tab-btn').forEach(b=>b.classList.remove('active')); document.querySelectorAll('.tab-panel').forEach(p=>p.classList.remove('active')); btn.classList.add('active'); document.getElementById(`tab-${name}`)?.classList.add('active'); if(name==='auditoria')renderAuditTable(); if(name==='solicitacoes')renderRequestsTab(); };
...
function setupRealtimeSync() {
    sb.channel('banco-rh')
        ...
        .on('postgres_changes', { event:'*', schema:'public', table:'holidays' }, async () => { await loadStaticComplianceData(); await refresh(); if($('holidays-modal')?.classList.contains('open')) renderHolidaysList(); })
        .on('postgres_changes', { event:'*', schema:'public', table:'vacations' }, async () => { await loadStaticComplianceData(); await refresh(); })
        .on('postgres_changes', { event:'*', schema:'public', table:'hr_settings' }, async () => { await loadStaticComplianceData(); await refresh(); })
        .on('postgres_changes', { event:'*', schema:'public', table:'bank_requests' }, async () => { await loadBankRequests(); renderRequestsTab(); updatePendingBadge(); })
        .subscribe();
}
...
// mês (competência) exibido no banco de horas, em vez de ser só informativo.
...
// Calendário do campo "Data de Referência" do ajuste manual — mesmo padrão
// visual/estrutural do calendário da tela Painel, com seleção de dia exato.
...
function showToast(title, type='success') {
    ...
    t.innerHTML=`<div class="toast-icon"><i class="fas ${icons[type]||'fa-check'}"></i></div><div class="toast-content"><p class="toast-title">${title}</p></div><button class="toast-close" onclick="this.closest('.toast').classList.add('hide');setTimeout(()=>this.closest('.toast')?.remove(),400)"><i class="fas fa-xmark"></i></button>`;
